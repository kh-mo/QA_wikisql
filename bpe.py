...
def apply_bpe(rule, args):
    infile = open(args.test_infile, "r", encoding="utf-8")
    outfile = open(args.test_outfile, "w", encoding="utf-8")
    for line in infile:
        outfile.write(bpe_process_line(line, rule, args))

def bpe_process_line(line, rule, args):
    '''
...
def segment_tokens(tokens, rule, args):
    output = []
    for token in tokens:
        ...
        new_word = [out for segment in _isolate_glossaries(token) for out in encode(segment,
                                                                                    self.bpe_rules,
