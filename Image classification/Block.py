...
def variable_initializer(init="", filter=[], output_channel=0, trainable=True, name=""):
...
def Conv(input, filter, output_channel, stride, init, trainable, name):
    with tf.variable_scope(name):
        filt = variable_initializer(init, filter, output_channel, trainable, name+"_filt")
        bias = variable_initializer(output_channel=output_channel, trainable=trainable, name=name+"_bias")
        conv = tf.nn.conv2d(input, filt, [1,stride,stride,1], padding="SAME") + bias
        return conv

def Act(input, function, name):
    if function == "relu":
        act = tf.nn.relu(input, name=name)
        return act

def Pool(input, filter, stride, function, name):
    with tf.variable_scope(name):
        if function == "max":
...
def BN(input, trainable, name):
    batch = tf.layers.batch_normalization(input, momentum=0.9, epsilon=0.001, training=trainable, name=name)
    return batch

def FC(input, output_channel, init, trainable, name):
    shape = 1
    for i, j in enumerate(input.get_shape().as_list()):
        if i > 0:
            shape *= j
    weight = variable_initializer(init, [shape], output_channel, trainable, name + "_filt")
    bias = variable_initializer(output_channel=output_channel, trainable=trainable, name=name + "_bias")
